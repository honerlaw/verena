...
import { Modal, Pressable, View, Dimensions, useWindowDimensions } from "react-native"
...
export const ContextMenu: React.FC<ContextMenuProps> = ({
  ...
  const [menuPosition, setMenuPosition] = useState<MenuPosition>({ top: 0, left: 0, placement: 'bottom' })
  const triggerRef = useRef<View>(null)

  // Calculate optimal menu position based on available space
  const calculateOptimalPosition = (triggerPos: typeof triggerPosition): MenuPosition => {
    ...
    const spaceBelow = height - (triggerPos.y + triggerPos.height) - screenPadding
    const spaceLeft = triggerPos.x - screenPadding
    const spaceRight = width - (triggerPos.x + triggerPos.width) - screenPadding

    // Determine best placement
    let placement: MenuPosition['placement'] = 'bottom'
    let top = triggerPos.y + triggerPos.height + gap
    let left = Math.max(screenPadding, triggerPos.x + triggerPos.width / 2 - menuWidth / 2)
    ...
    // Check if menu fits to the right
      ...
      placement = 'right'
      top = Math.max(screenPadding, triggerPos.y + triggerPos.height / 2 - menuHeight / 2)
    ...
    left = Math.max(screenPadding, Math.min(left, width - menuWidth - screenPadding))
    top = Math.max(screenPadding, Math.min(top, height - menuHeight - screenPadding))
